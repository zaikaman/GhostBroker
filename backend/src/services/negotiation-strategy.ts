/**
 * Negotiation strategy normalization.
 *
 * This module owns the AI-first split between what an operator
...
export function buildTurnContext(input: {
  ...
  const trustLevel: NegotiationTurnContext["trustLevel"] =
  ...
  return {
    ...
    ...(input.priorMoveRationale !== undefined ? { priorMoveRationale: input.priorMoveRationale } : {}),
  };
}
...
export function validateAgentDecision(
  ...
  if (action === "request_disclosure") {
    ...
    if (!claimType) {
      return {
        accepted: restatedHold(move, ctx, "No outstanding required claim to request; holding instead."),
  ...
  const accepted: AgentDecisionMove = {
    ...
    strategicIntent: move.strategicIntent ?? defaultIntentFor(action),
    confidence: clampConfidence(move.confidence),
    escalationRequested: Boolean(move.escalationRequested),
