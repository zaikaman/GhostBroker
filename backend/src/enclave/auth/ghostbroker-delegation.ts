 ...
 *
 *    For hand-crafted VCs that still use the legacy `did:t3n:0x<addr>`
 *    issuer format (or any other case where the SDK rejects the
 *    signature because the recovered signer does not match the
 *    issuer DID's embedded address), the verifier runs a
 *    multi-signer fallback that does the same ECDSA math the SDK
 *    does but accepts additional trusted signer addresses supplied
 *    by the composition root. Production server-minted VCs have
 *    `signer == issuer` (the tenant signing keypair is the
 *    institution's long-lived identity loaded via
 *    `TENANT_SIGNING_PRIVATE_KEY` or generated by
 *    `loadOrCreateTenantIdentity`; the T3N bearer API key is
 *    NOT used as the signing key — see `tenant-identity-store.ts`
 *    for the C1 fix), so the SDK call returns `isValid: true`
...
export interface GhostbrokerVerificationRequest {
   ...
   *
   * Production: server-minted VCs use the institution's
   * long-lived signing keypair (loaded via
   * `TENANT_SIGNING_PRIVATE_KEY` from a secret manager, or
   * generated by `loadOrCreateTenantIdentity` on first boot).
   * The keypair's derived address is the VC's `did:ethr:`
   * issuer, so `signer == issuer` and the SDK call returns
   * `isValid: true`. The fallback is unused on the happy path.
   */
 ...
 * verifies. In production (where the signer's keypair derived
 * address is the canonical `did:ethr:` issuer — see
 * `tenant-identity-store.ts`), the issuer is already
 * `did:ethr:` compatible, and the
